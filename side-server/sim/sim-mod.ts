/**
 * Default exact format IDs that permit passing turns (e.g. for catching mechanics,
 * throwing Poké Balls, or custom game modes).
 */
export const DEFAULT_PASS_BYPASS_FORMATS = [
	'gen9pokerogueclassic',
	'gen9pokerogueclassicdoubles',
	'gen9pokeroguerandom',
	'gen9pokeroguerandomdoubles',
] as const;

export const passBypassFormats = new Set<string>(DEFAULT_PASS_BYPASS_FORMATS);

export const SideMod = {
	passBypassFormats,

	/**
	 * Register one or more exact format names or format IDs to allow passing turns.
	 * Formats are converted to standard IDs (lowercase alphanumeric) to ensure exact matches.
	 */
	...
	/**
	 * Remove one or more exact format names or format IDs from the pass bypass registry.
	 */
	...
	isPokeRogueBypass(formatId: string): boolean {
		return this.isPassBypassFormat(formatId);
	},
...
export const SideServerMod = SideMod;
