...
> **Side Mod Extensions:** This server features a custom mechanics mod that allows Pokémon to optionally start battles with a specific HP percentage, a major status condition, custom Base Stat (BST) percentage boosts, and HP multipliers (HPX). These custom properties are fully supported across all three team formats.
...
**Custom Side Mod Sets:**
When utilizing custom server mechanics, you can add `HP`, `Status`, `BST`, and `HPX` lines directly into the export format:
...
| `HP` *(Side Mod)* | Left blank for 100% |
| `STATUS` *(Side Mod)* | Left blank for no status |
| `BSTBOOSTS` *(Side Mod)* | Formatted as `atk:def:spa:spd:spe`. Left blank if unmodified |
| `HPMULTIPLIER` *(Side Mod)* | Left blank for 1× |
